...
- Need list of all keys: English and Lithuanian
  - Use of a map to iterate over?
  - Data: Main key, Alt key, Caps?, Row?, special || regular
- Implement using Classes.
  - Keyboard Class
  - Key Class
    - Regular keys (letters, symbols, numbers)
    - Special keys (Ctrl, Alt, Shift, Enter...)
- Bonuses:
...
- Keyboard Class.
  ...
  - Constructor: Array of keys
    - State: Caps, Lang, Shift, Alt, Ctrl
  - Methods:
    - CreateKeys():
      - Iterate over keys, creating new Key class for each one.
        - check if regular or special.
    - CreateHtml():
      - Will iterate over keys, generating Html for each and inserting into relevent place.
      - CSS:
        - CSS class = map.key
        - Will also check if special or not
      - Will look at row to pick the row.
        - Will check to see if row class exists, if not will create new row.
    - AddEventListeners():
      - Iterate over keys, add event listener to each Key
        - Keys will have addEventListener method that will attach the passed function to the HTML
      - Will need to add onClick and keyDown triggers for each key
        - keyDown is important for shift and multiple key combos.
        - Thus, it is important to also immplement keyUp to disengage some settings
    - ChangeKeys():
      - Iterate over keys, changing based on current state
        - Shift, Ctrl
    - ChangeLang():
      - Iterate over keys, changing based on language.
...
- Key Class:
  ...
  - Methods:
    - addEventListener():
      - Takes in function and trigger from Keyboard and attaches it to the above html
    - getHTMLElement(): ?
      - Query selector that gets the HTML element.
=================================================
